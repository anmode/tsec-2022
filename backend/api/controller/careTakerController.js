...
exports.registerCareTaker = async (req, res, next) => {
    try {
        ...
        if( !name || !email || !contact || !password){
            res.status(200).send({ status: 400, message: "Make sure no field is empty" });
            return;
        }
...
exports.addUser = async(req,res,next) => {
    try {
        ...
        res.status(200).send({ status: 200, data: { newUser: newUser }, message: "Saved" });
        return;
    } catch (error) {
        next(error)
    }
...
exports.deleteUser = async(req,res,next) => {
    try{
        ...
        .then(() => res.status(200).send({ status: 200, message: "User Deleted" }))
        .catch((error) => {
            console.log(error);
            res.sendStatus(400);
        })
